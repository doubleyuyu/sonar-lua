...
public class FunctionWithTooManyParametersCheck extends SquidCheck<LexerlessGrammar> {
  ...
  @Override
  public void init() {
    subscribeTo(LuaGrammar.PARLIST);
  }

  @Override
  public void visitNode(AstNode astNode) {
    int nbParameters = astNode.getChildren(LuaGrammar.NAMELIST, LuaPunctuator.ELLIPSIS).size();
    if (nbParameters > max) {
      getContext().createLineViolation(this, "This function has {0,number} parameters, which is greater than the {1,number,integer} authorized.",
